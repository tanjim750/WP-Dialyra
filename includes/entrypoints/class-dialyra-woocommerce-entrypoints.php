...
class Dialyra_WooCommerce_Entrypoints {
	...
	public function handle_new_order( $order_id, $order = null ) {
		$this->debug_log_entrypoint( 'woocommerce_new_order', $order_id );
	...
	public function handle_checkout_order_created( $order ) {
		...
		$this->debug_log_entrypoint( 'woocommerce_checkout_order_created', $order_id );
		$this->dispatch_order_created( $order_id );
	}
	...
	public function handle_store_api_checkout_order_processed( $order ) {
		$order_id = $this->get_order_id( $order );
		$this->debug_log_entrypoint( 'woocommerce_store_api_checkout_order_processed', $order_id );
		$this->dispatch_order_created( $order_id );
	}
	...
	public function handle_checkout_update_order_meta( $order_id, $data = null ) {
		$this->debug_log_entrypoint( 'woocommerce_checkout_update_order_meta', $order_id );
		$this->dispatch_order_created( $order_id );
	}
	...
	public function handle_admin_order_saved( $order_id, $post = null ) {
		$this->debug_log_entrypoint( 'woocommerce_process_shop_order_meta', $order_id );
		$this->dispatch_order_created( $order_id );
	}
	...
	public function handle_payment_complete( $order_id ) {
		$this->debug_log_entrypoint( 'woocommerce_payment_complete', $order_id );
		$this->dispatch_order_created( $order_id );
	}
	...
	public function handle_thankyou( $order_id ) {
		$this->debug_log_entrypoint( 'woocommerce_thankyou', $order_id );
		$this->dispatch_order_created( $order_id );
	}
	...
	public function handle_order_updated( $order_id, $order = null ) {
		$this->debug_log_entrypoint( 'woocommerce_update_order', $order_id );
		$this->dispatch_order_created( $order_id );
	}
	...
	private function dispatch_order_created( $order_id ) {
		...
		$this->debug_log_entrypoint( $hook_name, $order_id );
	...
	/**
	 * Log entrypoint execution while WordPress debug mode is enabled.
	 *
	 * @since    1.0.0
	 * @param    string    $hook_name    Hook that fired.
	 * @param    int       $order_id     WooCommerce order ID.
	 */
	private function debug_log_entrypoint( $hook_name, $order_id ) {
		...
		error_log(
			sprintf(
				'WP Dialyra: entrypoint fired [%s] for order #%d',
